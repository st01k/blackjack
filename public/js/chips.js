function genChips() {
    var count = 0;
    // "yellow", "orange", "green", "blue", "grey"
    var colors = ["#edc92f", "#f77440", "#3a905e", "#4093ef", "#4d4f4f"];
    var values = [5, 25, 100, 500, 1000];
    var labels = ["5", "25", "100", "500", "1K"];
    var x = 245;
    var y = 300;
    ...
    d3.selectAll(".chip")
        .each(function() {
            ...
            chip.html(genChip(colors[count], labels[count], x, y));
            count++;
        });
}

function genChip(color, label, textX, textY) {
    chip =
        ...
        '<path style="fill:' + color + ';" d="M248.083,0.003C111.071,0.003,0,111.063,0,248.085c0,137.001,111.07,248.07,248.083,248.07 c137.006,0,248.076-111.069,248.076-248.07C496.159,111.062,385.089,0.003,248.083,0.003z"/>' +
        ...
        '<path style="fill:' + color + ';" d="M248.082,91.37c-86.55,0-156.714,70.156-156.714,156.713c0,86.543,70.164,156.707,156.714,156.707 c86.547,0,156.709-70.163,156.709-156.707C404.791,161.526,334.629,91.37,248.082,91.37z"/>' +
        '<g>' +
        ...
        '</g>' +
        '<text x="' + textX + '" y="' + textY + '" text-anchor="middle">' + label + '</text>' +
        '</svg>';
